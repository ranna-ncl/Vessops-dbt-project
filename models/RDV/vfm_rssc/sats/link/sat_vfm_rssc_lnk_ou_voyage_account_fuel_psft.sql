...
from {{ ref('ob_vfm_rssc_fuel_psft') }}

{% if is_incremental() %}
where not exists (
  select 1
  from {{ this }} t
  where t.hk_lnk_ou_voyage_account = {{ ref('ob_vfm_rssc_fuel_psft') }}.hk_lnk_ou_voyage_account
    and t.hashdiff = {{ ref('ob_vfm_rssc_fuel_psft') }}.hashdiff
)
{% endif %}
